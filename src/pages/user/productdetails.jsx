...
const ProductDetail = () => {
  ...
  const fetchRelatedProducts = async (category) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_URI}/products?category=${category}`);
      const data = await response.json();
      if (data.success) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_URI}/cart/addtocart`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
